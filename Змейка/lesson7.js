 ...
 */
function scoreField() {
    var s = document.getElementById('score-field');
    s.innerHTML = '<h3>Score: ' + score + '</h3>';
}
...
function startGame() {
    ...
    var mineInterval = setInterval(createMine, 8000);
    if (snake.length > FIELD_SIZE_X * FIELD_SIZE_Y / 100 * 90) {
...
function createMine() {
    ...
    while (!mineCreated) {
        ...
        if (!mine_cell_classes.includes('snake-unit') && !mine_cell_classes.includes('food-unit')) {
            ...
            mineCreated = true;
            if (snake.length < FIELD_SIZE_X * FIELD_SIZE_Y / 2) {
                setInterval(() => mine_cell.setAttribute('class', classes + ''), 20000);

            } else if (snake.length > FIELD_SIZE_X * FIELD_SIZE_Y / 2) {
                setInterval(() => mine_cell.setAttribute('class', classes + ''), 15000);
            } else if (snake.length > FIELD_SIZE_X * FIELD_SIZE_Y / 100 * 75) {
                setInterval(() => mine_cell.setAttribute('class', classes + ''), 5000);
            }
        }
    }
